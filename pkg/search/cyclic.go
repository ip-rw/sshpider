...
func ExpandNetworkToChan(network *net.IPNet) chan string {
	var targets = make(chan string, 1)
	go func() {
		...
		currNum = firstNum
		for {
			nextHost, err := cidr.Host(network, int(currNum))
			if err != nil {
				//logrus.WithError(err).Debugf("Bad network: %s", network)
				//break
			}
			if n := nextHost; n != nil {
				targets <- nextHost.String()
			}
			next(&cycle, &currNum)
